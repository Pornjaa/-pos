...
const ProductManager: React.FC<ProductManagerProps> = ({ products, onSave, onDelete, onBack, prefillData, persona = 'GRANDMA' as AiPersona, onScanningStateChange }) => {
  ...
  useEffect(() => {
    if (onScanningStateChange) onScanningStateChange(isCapturingProduct);
            ...
            products.map(p => {
              ...
              return (
                <div key={p.id} className={`bg-white p-5 rounded-[32px] border ${isLow ? 'border-red-200' : 'border-gray-100'} flex items-center gap-5 shadow-sm relative overflow-hidden`}>
                  {isLow && <div className="absolute top-0 left-0 bg-red-500 text-white text-[8px] font-black px-3 py-1 rounded-br-xl flex items-center gap-1 uppercase"><AlertTriangle size={8}/> ของใกล้หมด</div>}
                  {p.isRetailProduct && <div className="absolute top-0 right-10 bg-orange-500 text-white text-[9px] font-black px-3 py-1 rounded-b-xl flex items-center gap-1 uppercase">ขายย่อย</div>}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className={`px-2 py-0.5 rounded-full text-[10px] font-black ${isLow ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-400'}`}>คงเหลือ: {p.stockQuantity}</div>
                    </div>
                  </div>
      ...
      <div className="bg-white p-8 rounded-[50px] shadow-2xl space-y-8 animate-in slide-in-from-bottom duration-500">
        ...
        <div className="flex justify-center">
          {productImageUrl ? (
            ...
            <button onClick={() => setIsCapturingProduct(true)} className="w-48 h-48 rounded-[50px] bg-gray-50 border-4 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 text-gray-300 active:scale-95"><Camera size={48} /><span className="text-[10px] font-black uppercase">แตะเพื่อถ่ายรูปของ</span></button>
          )}
        </div>
        <div className="space-y-6">
          <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 uppercase ml-4">ชื่อสินค้า</label><input value={productName} onChange={e => setProductName(e.target.value)} className="w-full bg-gray-50 p-6 rounded-[30px] text-xl font-black border-2 border-transparent focus:border-blue-500 outline-none shadow-inner" placeholder="ของชื่ออะไรจ๊ะ..." /></div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1"><label className="text-[10px] font-black text-orange-400 uppercase ml-4">ทุนยาย</label><input type="number" value={costPrice} onChange={e => setCostPrice(e.target.value)} className="w-full bg-gray-50 p-6 rounded-[30px] text-2xl font-black text-center" placeholder="0" /></div>
            <div className="space-y-1"><label className="text-[10px] font-black text-blue-400 uppercase ml-4">ราคาขาย</label><input type="number" value={sellingPrice} onChange={e => setSellingPrice(e.target.value)} className="w-full bg-gray-50 p-6 rounded-[30px] text-2xl font-black text-center" placeholder="0" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1"><label className="text-[10px] font-black text-emerald-400 uppercase ml-4">จำนวนเริ่มต้น</label><input type="number" value={stockQuantity} onChange={e => setStockQuantity(e.target.value)} className="w-full bg-gray-50 p-6 rounded-[30px] text-2xl font-black text-center" placeholder="0" /></div>
            <div className="space-y-1"><label className="text-[10px] font-black text-red-400 uppercase ml-4">เตือนของน้อย</label><input type="number" value={minStockLevel} onChange={e => setMinStockLevel(e.target.value)} className="w-full bg-gray-50 p-6 rounded-[30px] text-2xl font-black text-center" placeholder="5" /></div>
          </div>
        </div>
        <button onClick={() => setIsRetailProduct(!isRetailProduct)} className={`w-full flex items-center justify-between p-6 rounded-[35px] border-4 transition-all ${isRetailProduct ? 'bg-orange-50 border-orange-500 text-orange-700 shadow-lg' : 'bg-gray-50 border-transparent text-gray-500'}`}>{isRetailProduct ? <CheckSquare size={32} /> : <Square size={32} />}<span className="text-xl font-black">เป็นสินค้าขายย่อย</span><ListFilter /></button>
